...
% T: arival time map
% D: distance map
% Max_Known_Dist: distance at which to exit
% SVr: seed positions
% unisotropy: wave speed unisotropy in a uniform intensity image
...
if length(sizeIM)==2
    sizeIM=[sizeIM,1];
    SVr=[SVr,ones(size(SVr,1),1)];
end

Im=zeros(sizeIM+2*pad);
Im(1+pad:end-pad,1+pad:end-pad,1+pad:end-pad)=IM;
clear IM
sizeIm=sizeIM+2*pad;
...
S=false(sizeIm); 
n_nonsimple = 1;
was_nonsimple=nan(10^5,1); %Increase this as needed for large images.
...
if debug_plots
    figure(100);
    imshow(Im(:,:,pad+1),[]);hold on;plot(SVr(:,2),SVr(:,1),'og','MarkerSize',10,'LineWidth',2)
end
...
while stop_cond
    ...
    if ~isempty(NewKnownPoint) && ~isinf(min_T)
        ...
        if debug_plots
            figure(100);
            plot(ny,nx,'sc','MarkerSize',5);hold on;drawnow
        end
        
        %KTcopy(was_nonsimple(1:n_nonsimple-1))=0;
        
        if simple3d(KTcopy(nx-1:nx+1,ny-1:ny+1,nz-1:nz+1)==1,6)==0
            S(NewKnownPoint)=1;
            was_nonsimple(n_nonsimple)=NewKnownPoint;
            n_nonsimple = n_nonsimple+1;
            KTcopy(NewKnownPoint)=0;
            plot(ny,nx,'xr','MarkerSize',10,'LineWidth',2);hold on;drawnow
        end
...
[nx,ny,nz] = ind2sub(size(KT),was_nonsimple(1:n_nonsimple-1));
nonsimple = [nx-pad,ny-pad,nz-pad];
